...
var powerDb = new sqlite3.Database(databaseFile, function (err) {
    ...
    if (err) {
        ...
        db.runSql("create table if not exists Probes ( id INTEGER primary key, Type text, CurrentChannel int check(CurrentChannel>=0 and CurrentChannel<=15), VoltageChannel int check(VoltageChannel>=0 and VoltageChannel<=5), Breaker int, Alert Text, SourceType int);", function (err) {
            if (err) {
                ...
                var afterReady = function () {
                    ...
                    db.runSql("Insert into Probes (id, Type, CurrentChannel, VoltageChannel, Breaker, Alert, SourceType) select 1,'SCT-006',0,0,0,20,null,0  where (select count(*) from Probes) = 0;", null, true);
                       ...
                       db.runSql("Alter table Probes add column Alert int;", function (err) {
                           if(err) {
                              ...
                              TableStates.Probes = "Error";
                               ...
                               db.runSql("Alter table Probes add column SourceType int;", function (err) {
                                   if (err) {
                                       console.log("Error adding SourceType column to Probes table: " + err);
                                       TableStates.Probes = "Error";
                                   } else {
                                       afterReady();
                                   }
...
var db =
{
  ...
  */
    ...
    updateProbe: function (id, type, currentChannel, voltageChannel, breaker, alert, probeType, callback) {

        var sql = '';

        if (id == null || id === undefined || id.toString() == '') {
            sql = "Insert into Probes Values(null,'" + type.escape() + "'," + currentChannel + ',' + voltageChannel + ',' + breaker + ",'" + alert + "'," + probeType + "); ";
        } else {
            sql = "Insert or replace into Probes Values(" + id + ",'" + type.escape() + "'," + currentChannel + ',' + voltageChannel + ',' + breaker + ",'" + alert + "'," + probeType + "); ";
        }
    ...
    updateProbes: function (probes, callback) {
        ...
        var f = function (index) {
            if (index < probes.length) {
                db.updateProbe(probes[index].id, probes[index].Type, probes[index].CurrentChannel, probes[index].VoltageChannel, probes[index].Breaker, probes[index].Alert, probes[index].SourceType, function (err, lastID) {
                    if (err) {
                        if (callback != null)
                            callback(err);
                    } else {
    ...
    replaceConfiguration: function (callback, configuration) {
        ...
        if (configuration != null) {
            ...
            for (index = 0; index < configuration.length; ++index) {
                ...
                sql += "Insert or replace into Circuits Values(" + val.id + "," + p1 + "," + val.CurrentChannel1 + ',' + val.VoltageChannel1 + "," + p2 + "," + val.CurrentChannel2 + ',' + val.VoltageChannel2 + ",'" + val.Name.escape() + "', '" + val.Description.escape() + "'," + val.Enabled + ',' + val.IsMain + ");"
            };
